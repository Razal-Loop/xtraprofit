...
const Header = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (isToolsOpen && !e.target.closest('[data-tools-dropdown]')) {
                setIsToolsOpen(false);
            }
        };
    ...
    const toolLinks = [
        { name: 'FX Parity', path: '/tools/currency', icon: <Globe size={14} />, desc: 'Live Currency Conversion' },
        { name: 'EMI Desk', path: '/tools/emi', icon: <ShieldCheck size={14} />, desc: 'Debt Amortization' },
        { name: 'Yield Forecast', path: '/tools/investment', icon: <Zap size={14} />, desc: 'Compound Growth' },
        { name: 'Profit Logic', path: '/tools/profit', icon: <Calculator size={14} />, desc: 'Margin Benchmarking' },
    ];
    ...
    const isToolActive = location.pathname === '/tools' || toolLinks.some(link => location.pathname === link.path);
        ...
        <>
            <header
                ...
                className={cn(
                    ...
                    scrolled
                        ? "bg-white/95 backdrop-blur-xl py-2.5 sm:py-3 border-b border-black/5 shadow-lg"
                        : "bg-white/80 backdrop-blur-md py-4 sm:py-6"
                )}
            >
                <nav className="flex justify-between items-center w-full px-4 sm:px-6 max-w-7xl mx-auto">
                    ...
                    <div className="hidden lg:flex items-center gap-6">
                        <div className="flex items-center gap-2 bg-black/5 p-1.5 rounded-full border border-black/5">
                            ...
                            >
                                ...
                                {/* Invisible hover bridge to connect trigger to dropdown */}
                                <div className={cn(
                                    "absolute top-full left-0 w-full h-4",
                                    isToolsOpen ? "block" : "hidden"
                                )} />

                                {/* Dropdown Menu */}
                                <div className={cn(
                                    "absolute top-[calc(100%+8px)] left-0 w-64 bg-white rounded-[1.5rem] shadow-2xl border border-black/5 p-3 grid grid-cols-1 gap-1 transition-all duration-300 origin-top-left z-[9999]",
                                    isToolsOpen
                                    ...
                                    <div className="border-t border-black/5 mt-1 pt-1">
                                        <Link
                                            to="/tools"
                                            className="flex items-center justify-center gap-2 p-3 rounded-[1rem] text-[0.55rem] font-black uppercase tracking-[0.25em] text-[#29695b] hover:bg-[#29695b]/5 transition-all"
                                        >
                                            View All Tools <ArrowUpRight size={12} />
                                        </Link>
                                    </div>
                                </div>
                            </div>
                    ...
                    {/* Mobile Menu Toggle */}
                    <button
                        className="lg:hidden p-2 sm:p-2.5 bg-[#00193c] text-white rounded-xl shadow-lg active:scale-90 transition-all font-body relative z-[10000]"
            ...
            {/* Mobile Nav Overlay — rendered outside header for independent z-stacking */}
            <div
                className={cn(
                    "lg:hidden fixed left-0 right-0 bottom-0 bg-white flex flex-col transition-all duration-300",
                    isMenuOpen
                ...
                <div className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col gap-6 border-t border-black/5">
                    ...
                    <div>
                        <Link
                            ...
                            className="flex items-center justify-between mb-6 pl-2 pr-2 group/header"
                        >
                            <p className="text-[0.6rem] font-black uppercase tracking-[0.4em] text-[#cbd5e1] group-hover/header:text-[#29695b] transition-colors">Calculators</p>
                            <span className="text-[0.5rem] font-black uppercase tracking-[0.2em] text-[#29695b] opacity-0 group-hover/header:opacity-100 transition-all flex items-center gap-1">View All <ArrowUpRight size={10} /></span>
                        </Link>
